...
__global__ void __launch_bounds__(128, kMinBlocks)
  ...
  using G2SCopyAS = typename Config::G2SCopyAS;
  ...
  while (true) {
    ...
    Tensor AS = make_tensor(
        make_gmem_ptr(xscale_ptr + uint64_t(m_tile_base)),
        make_shape (Int<kTileM>{}, ntile),
        make_stride(Int<1>{},      m_pad));
    Tensor gAS = local_tile(AS, make_tile(Int<kTileM>{}, Int<1>{}),
                            make_coord(_0{}, _));    // (kTileM, 1, ntile)
    ...
    G2SCopyAS g2s_tiled_copy_as;
    auto g2s_thr_as = g2s_tiled_copy_as.get_slice(idx);
    auto tgAS = g2s_thr_as.partition_S(gAS);    // (CPY, CPY_M=1, CPY_K=1, ntile)
    auto tsAS = g2s_thr_as.partition_D(sAS);    // (CPY, CPY_M=1, CPY_K=1, kStage)

    auto pred_xs = make_tensor<bool>(
        make_shape(shape<0>(tgAS), shape<1>(tgAS), shape<2>(tgAS)));
    const bool xs_live = (idx < Config::kXsLiveThrs);
#pragma unroll
    for (int i = 0; i < size(pred_xs); ++i) pred_xs(i) = xs_live;
...
#pragma unroll
    ...
    {
      constexpr int kWsVecs = kMaxNtileK / 4;   // 32
      if (idx < kWsVecs) {
        int k_vec = idx * 4;
...
#pragma unroll
    for (int i = 0; i < kStage - 1; ++i) {
      if (i < ntile) {
        ...
        cute::copy_if(g2s_tiled_copy_as, pred_xs,
                      tgAS(_, _, _, i), tsAS(_, _, _, i));
        ++itile_to_read;
        ++ismem_write;
      }
      cp_async_fence();
    ...
    for (int itile = 0; itile < ntile; ++itile) {
      if (itile_to_read < ntile) {
        ...
        cute::copy_if(g2s_tiled_copy_as, pred_xs,
                      tgAS(_, _, _, itile_to_read),
                      tsAS(_, _, _, ismem_write));
        ++itile_to_read;
...
#pragma unroll
      for (int in = 0; in < kN; ++in) {
        int m_idx = get<1>(tI_mn(0, in));
        tCS[in] = sAS(m_idx, 0, ismem_read) * wscale_val;
      }
...
#pragma unroll
      ...
      // Apply per-column scale and accumulate into tDr.
      // mul blockwise scale 
      // dequant 
      auto tDr_mn = retile_fragment(tDr);
#pragma unroll
      for (int in = 0; in < kN; ++in) {
        const float yscale = tCS[in];
#pragma unroll
...
void group_gemm_fp8_blockwise_multistage_async(
  ...
  assert((k / 128) <= 128 &&
         ...
         "(raise kMaxNtileK in config to relax)");
